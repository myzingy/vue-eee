        ...
        <div slot="main">
            ...
            <div >
                ...
                <el-form :inline="true" :model="searchArgs" class="demo-form-inline">
                    ...
                    <el-form-item :label="$t('支付状态')" prop="render_payment_status">
                        <el-select v-model="searchArgs.render_payment_status" clearable @change="onSearch" :placeholder="$t('请选择')">
                            <el-option
                ...
                <el-table :data="templateList" border style="width: 100%" @selection-change="checkTemplateItem">
                    ...
                    <el-table-column prop="render_payment_status"  :label="$t('支付状态')">
                        <template slot-scope="scope">
                            {{templateConfigs.payment_type[scope.row.render_payment_status]}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="render_type" :label="$t('渲染类型')"> </el-table-column>
                    <el-table-column prop="render_price" :label="$t('渲染价格')"> </el-table-column>

                    <el-table-column :label="$t('操作')">

                        <template slot-scope="scope">
                            <el-button-group>
                                <el-button type="primary" v-if="scope.row.render_price<=0" @click.native="editPrice(scope.row)" icon="el-icon-edit">{{$t('录入价格')}}</el-button>
                            </el-button-group>
                        </template>
            ...
            <el-dialog :title="$t('录入价格')" :visible.sync="dialogFormVisible">
                <el-form ref="form" :model="form" :rules="formRules" label-width="110px">
                    ...
                    <el-form-item :label="$t('渲染类型')" prop="render_type">
                        <el-select v-model="form.render_type" :placeholder="$t('请选择')">
                            <el-option
                                    v-for="(item, index) in templateConfigs.render_type"
                                    :key="index"
                                    :label="item"
                                    :value="index">
                            </el-option>
                    ...
                    <el-form-item :label="$t('渲染价格')" prop="render_price">
                        <el-input v-model="form.render_price"></el-input>
                    </el-form-item>
                    <el-form-item>
...
<script>
    ...
    let App ={
        ...
        methods : {
            ...
            getLists(){
                this.searchArgs = Object.assign(this.searchArgs,{list_type:'studio_price_list'});
            ...
            setPaid(){
                ...
                vk.http(uri.productTemplateSetPaid,{type:'studio',ids:this.multipleSelection},this.then);
            },
            onSubmit(formName) {
                let that = this;
